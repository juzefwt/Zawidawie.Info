...
class AddAdAcceptanceTest extends AbstractAcceptanceTest
{
    public function testPostCreationPageShowsAForm()
    {
        ...
        $this->assertEquals(1, $crawler->filter('#ad_form')->count());
        $this->assertEquals('Zapisz', $crawler->filter('#ad_form .submit')->attr('value'));
    }
	...
	$title = 'What the heck - '.uniqid();
        ...
        $crawler = $this->requestPostCreationPage($client);
        $form = $crawler->selectButton('Zapisz')->form();
        $form['AdType[title]'] = $title;
        $form['AdType[content]'] = $message;
        $client->submit($form);
    ...
    {
        $url = $this->generateUrl($client, 'ads_new');
